...
def collate_fn_limit(batch, max_batch_points, logger):
    coord, xyz, feat, label, files,image = list(zip(*batch))
    offset, count = [], 0
    
    new_coord, new_xyz, new_feat, new_label,new_image = [], [], [], [], []
    k = 0
    ...
    new_label_dict = {}
    for scale_name in new_label[0].keys():
        tmp = []
        for i in range(k):
            tmp.append(new_label[i][scale_name])
        new_label_dict[scale_name] = torch.cat(tmp)

    return torch.cat(new_coord[:k]), torch.cat(new_xyz[:k]), torch.cat(new_feat[:k]),new_label_dict, torch.IntTensor(offset[:k]), files, torch.cat(new_image[:k])
...
def collation_fn_voxelmean(batch):
    ...
    """
    coords, xyz, feats, labels, inds_recons, files, image = list(zip(*batch))
    ...
    new_label_dict = {}
    for scale_name in labels[0].keys():
        tmp = []
        for i in range(len(labels)):
            tmp.append(labels[i][scale_name])
        new_label_dict[scale_name] = torch.cat(tmp)
    ...
    return coords, xyz, feats, labels, offset, inds_recons, files, image
